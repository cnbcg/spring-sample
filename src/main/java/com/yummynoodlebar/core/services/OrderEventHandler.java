...
public class OrderEventHandler implements OrderService {
	...
	public OrderCreatedEvent createOrder(CreateOrderEvent createOrderEvent) {
		...
		OrderStatusEvent orderStatusEvent = ordersPersistenceService.setOrderStatus(new SetOrderStatusEvent(order.getKey(), new OrderStatusDetails(
				order.getKey(), UUID.randomUUID(), new Date(), "Order Created")));
