...
public class OrderStatusIntegrationTest {
	...
	public void thatOrderViewIsForwardedTo() throws Exception {
		mockMvc.perform(get("/order/" + uuid)).andExpect(status().isOk()).andExpect(forwardedUrl(ORDER_VIEW));
	}
	...
	public void thatOrderStatusIsPutInModel() throws Exception {
		...
		verify(orderService).requestOrderDetails(
				Matchers.<RequestOrderDetailsEvent> argThat(org.hamcrest.Matchers.<RequestOrderDetailsEvent> hasProperty("key", equalTo(uuid))));
