...
public class SiteControllerIntegrationTest {
	...
	public void setup() {
		...
		when(menuService.requestAllMenuItems(any(RequestAllMenuItemsEvent.class)))
		.thenReturn(allMenuItems());
	}
	...
	public void rootUrlPopulatesViewModel() throws Exception {
		mockMvc.perform(get("/"))
				...
				.andExpect(model().attribute("menuItems", hasItems(hasProperty("name", equalTo(STANDARD)), 
																	hasProperty("name", equalTo(CHEF_SPECIAL)),
																	hasProperty("name", equalTo(LOW_CAL)))))
